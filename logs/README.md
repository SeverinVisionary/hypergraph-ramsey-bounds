...
| `verify_n83_444.log` | complete independent verification of the `R(4,4,4;3) >= 84` witness by `verify_witness_n83.py`, bound to the colour vector's SHA-256: all 1,837,620 four-subsets scanned, zero monochromatic in any of the three colours, and a perturbation control that must produce a violation. |
...
| `verify_n26_554.log`, `verify_n31_554.log`, `verify_n32_554.log`, `verify_n33_554.log`, `verify_n34_554.log` | the five smaller `R(5,5;4)` witnesses under the same two independent checkers as the n=35 object, each bound to its colour vector's SHA-256. With these and the n=79..82 runs, every one of the twelve deposited witnesses has an independent hash-bound verification log. |
